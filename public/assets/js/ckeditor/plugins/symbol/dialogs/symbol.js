    ...
    function o(a, b, c) {
        if (null != a) {
            for (var s = CKEDITOR.tools.getNextId() + "_symbol_table_label", i = [""], h = 0, d = 0, j = !1, f, g, k = a.split(","), l = 0; l < k.length; l++) {
                ...
                h += a - f + 1;
                if (a >= f) for (j || (i.push('<table role="listbox" aria-labelledby="' + s + '" style="width: 100%; height: 100%; border-collapse: separate;" align="center" cellspacing="2" cellpadding="2" border="0">'), j = !0); f <= a;) {
                    0 == d && i.push("<tr>");
    ...
    return {
        title: "Select Symbol", buttons: x, charColumns: 16, onShow: function () {
            ...
            if (0 < a.childNodes.length) {
                var b = null;
                n || (b = "Basic Latin", j.config.defaultSymbolRange && (b = j.config.defaultSymbolRange), console.log("startRange = " + b));
                for (var c = 0, d = 0; d < a.childNodes.length; d++) if (console.log(d + " domElement.childNodes[i].text = " + a.childNodes[d].text),
                a.childNodes[d].text == b || a.childNodes[d].value == n) {
                    c = d;
                    break
                }
                a.selectedIndex = c;
